...
| Prompt-injection evaluation loop for ChatGPT models (corpus + baseline + scoring) | `rules/ai-workflow-policy.md` §8.1 (CT-15), `rules/security/injection-eval-spec.md` | `rules/security-policy.md` §19 (PI-5.2), `security-evals/run_injection_evals.py` |
...
| LLM output validation constraints (dates, arithmetic, API args, logical constraints — external verification required) | `rules/ai-workflow-policy.md` §llm-output-validation-constraints | `4-ml-systems-mlops/evaluation-debugging/llm-reasoning-failures/llm-reasoning-failures-and-temporal-blunders.md`, `4-ml-systems-mlops/evaluation-debugging/llm-reasoning-failures/llm-limitations-mitigation-patterns.md` |
| Tool call argument validation (hallucinated API args must abort, not retry) | `rules/agent-stopping-conditions.md` | `gorilla-llm-connected-with-massive-apis.pdf` (in llm-reasoning-failures/tool-use/) |
...
| OBSERVE stage (mandatory post-session logging of tokens/USD, latency/calls, incidents) | `rules/ai-workflow-policy.md` §Explicit Operating Contract (`### Enforcement`) | `rules/token-cost-controls.md`, `rules/agent-stopping-conditions.md`, `rules/templates/.cursorrules`, `rules/daily-navigation.md` §Session End |
...
| OWASP Top 10:2025 web application coverage matrix (traceability; not app certification) | `rules/security-policy.md` Appendix C | [OWASP Top 10:2025](https://owasp.org/Top10/2025/), Appendix A (LLM Top 10) |
| Browser-facing quality gates (Tier A/B/C, Observatory + Lighthouse evidence) | `rules/web-policies.md` (Browser-Facing Quality Gates) | [HTTP Observatory](https://developer.mozilla.org/en-US/observatory/), [Lighthouse](https://developer.chrome.com/docs/lighthouse/overview), [Web Almanac 2025 Performance](https://almanac.httparchive.org/en/2025/performance) (non-normative targets) |
| Tier A content UX review (NN/g habits; Baymard for ecommerce flows) | `rules/web-policies.md` (Content UX checklist, non-normative) | [NN/g Top 10 Web Design Mistakes](https://www.nngroup.com/articles/top-10-mistakes-web-design/), [Baymard UX Benchmark](https://baymard.com/ux-benchmark) |
...
| Automated prompt-injection eval harness (static validation + runtime baseline scoring) | `rules/security/injection-eval-spec.md` | `rules/security-policy.md` §19 (PI-5.2), `security-evals/attack-corpus/`, `security-evals/baselines/injection-baseline.json` |
