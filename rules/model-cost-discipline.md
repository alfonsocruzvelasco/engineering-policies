...
4. **Spend freeze overrides upgrade paths.** While `approved-ai-tools.md` contains `SPEND FREEZE (active)`, do not upgrade a workflow to a billed model, enable extra usage, or add a payment method. Cheaper-first evaluation still applies after the freeze is lifted.
