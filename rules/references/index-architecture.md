...
- `gemma-4-12b-architecture` — Google (June 3 2026). Encoder-free multimodal architecture: 35M vision embedder (single matmul, factorized coordinate lookup) replaces 27 vision transformer layers; raw 16kHz audio projected directly to LLM input space without separate encoder. Requires 16GB VRAM for local inference — above RTX 4070 limit, RunPod evaluation only. Key architectural implication for CV fine-tuning: single LoRA pass updates all modalities simultaneously, eliminating frozen encoder constraint of traditional VLMs. Reference: https://developers.googleblog.com/gemma-4-12b-the-developer-guide/
