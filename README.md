...
This corpus is large on purpose. The usual failure modes are getting lost, copying it without adapting it, letting policies go stale, or applying full production discipline where a lighter path fits. **Normative rules still live only under `rules/`** — this section is orientation and upkeep, not policy text.

### Where to start

1. **[`rules/system/concept-index.md`](rules/system/concept-index.md)** — Maps a concept to the **authoritative** policy file. When two documents touch the same topic, the index decides what governs.
2. **This README** — Orientation and the `/rules` index (what each major file is for).
3. **Topic-specific policies** — Open only the file the concept index lists for your question.

**If documents disagree:** Follow the authority model in `rules/production-policy.md` (standalone policies override navigation sections inside merged files when they conflict). For security versus other domains, **`rules/security-policy.md` wins** where it explicitly claims precedence (see also `rules/dependency-install-policy.md`).

### Reading paths (by intent)

Suggested entry sequences, not requirements:
...
### When you do not need the full corpus

**Learning-only or throwaway work** is governed by [`rules/system/learning-library-governance.md`](rules/system/learning-library-governance.md) and [`rules/system/learning-ai-usage-boundary.md`](rules/system/learning-ai-usage-boundary.md). Use those for repos under `~/learning-repos/` and similar. You are not expected to load every policy into a one-off tutorial or notebook.
...
### Adopting or forking (portability)

This corpus includes **example org placeholders** (for example `security@organization.com`), **personal canonical paths** in this README, and **review dates** inside registries. If you fork or reuse it:

- Replace placeholder contacts and ownership with your team’s identifiers.
- Treat `~/dev/...` paths as **conventions**, not universal law; align with `rules/development-environment-policy.md` or rewrite the README path block to match your layout.
...
**Depth varies:** Some files are comprehensive; others are pointers or merged bundles. Use `concept-index.md` to find the **authoritative** layer for a topic instead of assuming every file is equally detailed.

### Reducing overlap over time
