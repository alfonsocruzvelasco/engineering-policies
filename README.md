...
The `/policies` folder is organized around **compiled policy bundles** (merged documents) to reduce fragmentation and maintenance overhead, with domain-specific policies split for better navigation.
...
- **`policies/production-policy.md`** **[SPLIT 2026-02-02]**
...
1. Review `development-environment-policy.md` for directory structure and workspace isolation
2. Review `production-policy.md` for data/storage rules, SQL discipline, and development environment setup
3. Review `language-policies.md` for language-specific standards (Python, TypeScript, Java, C++, Rust, CUDA)
4. Review `testing-policy.md` for testing standards and CI/CD integration
5. Review `ai-systems-architecture-policy.md` for LLM/AI system architecture patterns
6. Review `mlops-policy.md` for experiment tracking, model serving, and monitoring setup
7. Review `ml-cv-operations-policy.md` for ML/CV-specific operations (model evaluation, feature engineering, data quality)
8. Review `production-policy.md` §8 (Git and Source Control) for Git workflow and branching model
9. Review `versioning-and-release-policy.md` for versioning schemes and release processes
10. Review `security-policy.md` for secrets handling and ML/CV security
11. Review `ai-workflow-policy.md` Part 1 for Cursor sandbox rules and daily workflow
12. Review `ai-workflow-policy.md` Part 4 for structured spec workflows (Spec Kit/OpenSpec/MCP)
13. Check `system/raid/` for RAID storage setup if working with large datasets
14. Use `templates/` for standard project structures and prompts (see `readme-template.md` for README with Technical Baseline)
...
5. **Container best practices** — See `policies/system/containers/` for Docker, Kubernetes, and Docker Compose patterns
